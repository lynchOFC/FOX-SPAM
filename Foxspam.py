...
{r}[{y}2{r}]{g} SPAM-GMAIL
    ...
    def escolha_main(self, esc):
            if esc == 1: 
                ...
                print(f'{r}DIGITE APENAS NUMEROS QUE ESTEJAM NAS OPÇÕES ACIMA!!!{n}')
                time.sleep(2)
                self.__init__()
    def main1(self):
        os.system('cls') and os.system('clear')
...
{y}(EXEMPLO DE COMO DEVE COLOCAR O NUMERO: +5518997961798){n}
        """)
        numero= (input(f'{g}Numero que recebera o spam:{n} '))
        if numero == '/exit':
            self.__init__()
        elif len(numero) < 14 or len(numero) > 15:
            print(f'{r}DIGITE O NUMERO COMO NO EXEMPLO ACIMA!!!{n}')
            time.sleep(2)
            self.main1()
